...
export function DemoClient({ initialProof }: { initialProof: ProofBundle }) {
  ...
  function rerunProof() {
    startTransition(async () => {
      ...
      try {
        ...
        if (!response.ok) throw new Error("The proof service could not complete this run.");
        setProof((await response.json()) as ProofBundle);
      } catch (error) {
  ...
  async function translateExplanation(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.error ?? "Translation could not be loaded.");
      setExplanation(body as Explanation);
    } catch (error) {
  ...
  async function runShadowAudit() {
    ...
    try {
      const response = await fetch("/api/shadow-audit", {
        ...
        body: JSON.stringify({ population_size: 200, capacity: 20, seed_runs: 10 }),
      });
      if (!response.ok) throw new Error("The shadow audit could not complete.");
      setShadow((await response.json()) as ShadowAuditReport);
    } catch (error) {
  ...
  async function runThreatReport() {
    ...
    try {
      ...
      if (!response.ok) throw new Error("The adversarial checks could not complete.");
      setThreats((await response.json()) as ThreatReport);
    } catch (error) {
      ...
      <section className="proof-section" id="proof">
        <div className="shell">
          ...
          <div className="proof-console">
            <div className="console-bar"><div><span className="live-dot" /> SYNTHETIC SCENARIO · 200 PEOPLE · 20 SLOTS</div><span>PROOF {proof.proof_version.replace("commonsgate-proof-", "").toUpperCase()}</span></div>
      ...
      <section className="evidence shell" id="evidence">
        ...
        <div className="evidence-grid">
          <article className="lab-card shadow-card">
            ...
            <code>{shadow?.report_hash ? `${shadow.report_hash.slice(0, 20)}…` : "Run to generate signed report hash"}</code>
          </article>
          ...
          <article className="lab-card threat-card">
            ...
            <div className="threat-footer"><span>{threats ? `${threats.passed_count}/${threats.total_count} executed checks passed` : "Ready to execute against the live API"}</span><code>{threats?.report_hash ? `${threats.report_hash.slice(0, 16)}…` : "NO CASE DATA"}</code></div>
          </article>
        </div>
      ...
      <section className="language-section" id="language">
        <div className="shell language-layout">
          ...
          <div className="language-demo">
            ...
            <div className="delivery-status" aria-live="polite"><span>Requested <b>{explanation?.requested_language ?? language}</b></span><Icon name="arrow" /><span>Delivered <b>{explanation?.delivered_language ?? "en"}</b></span><span className={explanation?.fallback_used ? "fallback" : "safe"}>{explanation?.fallback_used ? "safe fallback" : "translated"}</span></div>
          </div>
        </div>
      </section>
